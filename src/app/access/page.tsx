...
const ACCESS_LEVELS = {
  operations: {
    ...
    icon: Settings,
    get password() { return process.env.DEMO_OPERATIONS_PIN || '1234'; },
    redirect: '/operations',
    highlights: [
  ...
  workshop: {
    ...
    icon: Wrench,
    get password() { return process.env.DEMO_WORKSHOP_PIN || '5678'; },
    redirect: '/workshop',
    highlights: [
...
export default function AccessPage() {
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    if (secureCompare(password, accessConfig.password)) {
      // Directly authenticate with NextAuth using demo credentials
      try {
        const result = await signIn('credentials', {
          email: selectedAccess === 'operations' ? 'ops@example.com' : 'workshop@example.com',
        ...
        <Card className="w-full max-w-md rounded-2xl border border-blue-100/60 bg-white/95 shadow-lg dark:border-blue-900/40 dark:bg-gray-950/70">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="rounded-xl border border-blue-100/80 bg-blue-50/60 p-4 text-left text-xs text-blue-900 dark:border-blue-900/40 dark:bg-blue-900/10 dark:text-blue-100">
              <p className="font-medium">Demo PINs</p>
              <p>Operations: 1234 | Workshop: 5678</p>
            </div>
